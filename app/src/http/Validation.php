...
class Validation
{
    ...
    public static function make($key, $value, $model)
    {
        $self = new self();
        $self->model = $model;
        $self->paramKey = $key;
    ...
    public function checker($rules, $value) {
        ...
        $isNull = isset($rules['null']) ? $rules['null'] == 'NULL' : $rules['null'] != 'NOT NULL';
    ...
    private function verifyType($rules, string $type, $value)
    {
        if($type == 'INT') {
            ...
            $date = $this->verifyDate($value);
            return $date ? $date : $this->assignErrors(is_not_date_error);
    ...
    private function verifyDate($value)
    {
        ...
        if(count($dateArray) != 3) {
            return $this->assignErrors(is_not_date_error);
    ...
    private function verifyDatetime($value)
    {
        ...
        if(count($datetime) == 2) {
            ...
            $date = $this->verifyDate($datetime[0]);
